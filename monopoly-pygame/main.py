from monopoly_board import Board
from space import Monopoly_Space
from space import Monopoly_Property
from space import Monopoly_Community_Chest
import os
...
class Game():
    ...
    def handle_button_logic(self, mouse, event):
        for button in self.buttons:
            mouse_over_button = (button.rect.x <= mouse[0] <= button.rect.x + button.rect.width and button.rect.y <= mouse[1] <= button.rect.y + button.rect.height)
    ...
    def draw_properties(self, properties, base_x : int, base_y : int, edge_case=True, player=None, no_stack=None):
        seen = []
    ...
    def draw_window(self):
        ...
        for text in self.texts:
            text.draw(self.WIN)
        if self.show_board:
            self.WIN.blit(self.board.IMAGE, (80,80))
            draw_all_players(self.players, self.WIN)
            self.draw_properties(self.player.properties,-55, 760, True, self.player)
            self.draw_properties(self.player_2.properties,-55, 90, player=self.player_2)
            self.draw_properties(self.player_3.properties,760, 110, player=self.player_3)
            self.draw_properties(self.player_4.properties,760, 760, True, self.player_4)
