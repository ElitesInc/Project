...
class Player:
    ...
    def __init__(self, color : Tuple, image):
        ...
        self.color = color
        self.rectangle = pygame.Rect(810, 810, 60,60)
        self.image = image
        #self.jail_turn = -1
        self.roll_num = None
    ...
    def move(self, roll : int, board : Board):
        # Calculate the new position without wrapping around
        new_position = self.position + roll

        # Check if the new position exceeds the board size (40 spaces)
        #if new_position >= 40:
            # Adjust the player's money for passing Go
            #self.money += 200
            # Wrap the position around to the beginning of the board
        self.position = new_position % 40
        #else:
            #self.position = new_position

        landed_on_space = board.places[self.position]
    ...
    def pay(self, game, roll=None, cpu=False):    
        ...
        is_tourism = landed_on_space.space_name == "Tourism Tax"
        #is_luxury = landed_on_space.space_name == "Luxury Tax"
        is_tax = is_tourism #or is_luxury
    ...
    board : Board, players):
        ...
        match r:
            ...
            case 8:
                self.money += 20
                return Text("Tourism tax refund. Collect $20.",
                y=Y_CONST) 
            case 9:
                for p in players:
                    if p is not self:
